...
namespace WMaster.Entity.Living.GangMission
{
    ...
    public abstract class GangMissionBase
    {
        ...
        /// <summary>
        /// Cost multiplier for this mission.
        /// </summary>
        private static SortedDictionary<EnuGangMissions, int> m_costFactor = new SortedDictionary<EnuGangMissions, int>()
        {
            {EnuGangMissions.Sabotage, 150},
            {EnuGangMissions.SpyGirls, 40},
            {EnuGangMissions.RecaptureGirls, 125},
            {EnuGangMissions.Extortion, 116},
            {EnuGangMissions.PettyTheft, 110},
            {EnuGangMissions.GrandTheft, 250},
            {EnuGangMissions.KidnappGirls, 150},
            {EnuGangMissions.Catacombs, 300},
            {EnuGangMissions.Training, 90},
            {EnuGangMissions.Recruit, 80},
            {EnuGangMissions.Service, 100},
            {EnuGangMissions.Guarding, 60},
            {EnuGangMissions.Dungeon, 150},
            {EnuGangMissions.None, 40}
        ...
        /// <summary>
        /// Get cost multiplicateur for mission.
        /// </summary>
        /// <param name="missionType">the <see cref="EnuGangMissions"/> type.</param>
        /// <returns>Cost factor of mission or default cost factor if mission wasn't configured. (Default = EnuGangMissions.Guarding).</returns>
        public static int GetCostFactor(EnuGangMissions missionType)
        {
            if (m_costFactor.ContainsKey(missionType))
            { return m_costFactor[missionType]; }

            WMLog.Trace(string.Format("No cost factor defined for mission'{0}'!", missionType), WMLog.TraceLog.ERROR);
            return GetCostFactor(EnuGangMissions.Guarding);
        }
