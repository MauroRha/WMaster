...
public:
	...
	void SetMultiline(bool multi, int width, int height){
		//m_IsMultiline=multi;m_Width = width;m_Height=height;
	}
	...
	int GetLinesPerBox(){
		//if(m_Lineskip>0)return (m_Height/m_Lineskip);else return m_Height/GetFontLineSkip();
		return -1;
	}
	...
	void LeftOrRight(bool leftorright){
